...
export function useServiceOverview() {
  ...
  const values = reactive({
    email: '', avatarCount: 0, applicationCount: 0, conversationCount: 0, tagState: '—',
    chatPolicy: '未读取', balance: '—', entryCount: 0, bidCount: 0, carouselCount: 0, topCoin: '—', rules: '—',
  })
  ...
  async function load() {
    ...
    try {
      await Promise.all([
        ...
        probe('identity', listAvatars, result => { values.avatarCount = result.length }),
        probe('content', getMyTag, result => { values.tagState = result.active?.state || result.draft?.state || '未发布' }),
        probe('content', () => listApplications({ direction: 'RECEIVED', limit: 1 }), result => { values.applicationCount = result.items.length }),
        probe('chat', () => listChats({ limit: 1 }), result => { values.conversationCount = result.items.length }),
        ...
        probe('wallet', () => listWalletEntries({ limit: 1 }), result => { values.entryCount = result.length }),
        probe('exposure', getCurrentExposure, result => { values.topCoin = result.paidCoin }),
        probe('exposure', () => listExposureBids({ limit: 1 }), result => { values.bidCount = result.items.length }),
        probe('exposure', () => getExposureCarousel({ limit: 1 }), result => { values.carouselCount = result.items?.length || 0 }),
  ...
  const identityRows = computed<SettingsRow[]>(() => [
    ...
    { id: 'avatars', symbol: '像', title: '头像目录', description: '可用头像与当前选择', value: `${values.avatarCount} 张` },
  ])
  const contentRows = computed<SettingsRow[]>(() => [
    { id: 'tag', symbol: '#', title: 'Tag 状态', description: '草稿、发布、问题与回答链路', value: values.tagState },
    { id: 'applications', symbol: '申', title: '申请记录', description: '收到、发出与处理进度', value: `${values.applicationCount} 条` },
  ...
  const walletRows = computed<SettingsRow[]>(() => [
    ...
    { id: 'ledger', symbol: '流', title: '钱包流水', description: '不可变收支记录', value: `${values.entryCount} 条` },
    { id: 'exposure', symbol: '顶', title: '当前置顶', description: 'TOP、轮播与到期时间', value: `${values.topCoin} Coin` },
    { id: 'bids', symbol: '竞', title: '竞价记录', description: '提交结果与幂等查询', value: `${values.bidCount} 条` },
    { id: 'rules', symbol: '规', title: '曝光规则', description: '起拍价、加价幅度与时长', value: values.rules },
  ])
  const chatRows = computed<SettingsRow[]>(() => [
    { id: 'chats', symbol: '聊', title: '聊天连接', description: '会话、Peer 解析与 Tinode 凭证', value: `${values.conversationCount} 个` },
